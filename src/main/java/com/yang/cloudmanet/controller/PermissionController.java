...
import com.yang.cloudmanet.utils.IPUtil;
...
public class PermissionController {
    ...
    private void updateUserlogin(HttpServletRequest request, NodeInfo user) throws Exception{
        String ip = IPUtil.getIpAddress(request);
        String ipaddress = "";
        String loginTime = timeParse.convertDate2String(new Date(),"yyyy-MM-dd HH:mm:ss");
        user.setPosition(ipaddress);
        user.setLoginTime(loginTime);
        permissionService.updateNodeInfo(user);
    }
    ...
    public Object logout(HttpSession session,HttpServletRequest request){
        try {
            Map<String, HttpSession> userMap = (Map<String, HttpSession>) request.getServletContext()
                    .getAttribute("userMap");
            NodeInfo userlogin = (NodeInfo) session.getAttribute("user");
            if (userlogin != null) {
    ...
    public Object UserLogin(@RequestBody String jsonData, HttpServletRequest request, HttpServletResponse response,
        ...
        try {
            ...
            if (userMap.get(username) == null) {
                ...
                NodeInfo cuser = permissionService.authenticate(param);
                if (cuser == null) {
